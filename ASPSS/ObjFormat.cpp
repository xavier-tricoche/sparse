...
ObjFaceHandle ObjSubMesh::createFace(uint nofVertices, Obj::Options att, int matId/*=-1*/)
{
...
//             std::cout << args.toVector3() << " ";
        ...
        else if ( (command=="usemtl" && usemtl!=args) || (command=="usemap" && usemap!=args))
        {
            ...
            if (usemtl!="")
            {
                ...
                {
                    std::cerr << "ObjLoader::Warbing - Material \"" << usemtl << "\" not found.\n";
                }
...
ObjMesh* ObjMesh::createIndexedFaceSet(Obj::Options options) const
{
    ...
    std::vector< std::map<long long int,uint> > vertices;
    vertices.resize(positions.size());
    
    for (uint smi=0 ; smi<mSubMeshes.size() ; ++smi)
    {
        ...
        for (uint fid = 0 ; fid<pSrcSubMesh->getNofFaces() ; ++fid)
        {
            ...
            if ( (!options&Obj::Triangulate) && (pSrcSubMesh->getConstNofVerticesPerFace()>0) )
            ...
            for (uint polyId=0 ; polyId<nofSubPolygons ; ++polyId)
            {
                ...
                for (uint k=0 ; k<3 ; ++k)
                {
                    ...
                    std::map<long long int,uint>::const_iterator vit = vertices[sface.vPositionId(ids[k])].find(key);
