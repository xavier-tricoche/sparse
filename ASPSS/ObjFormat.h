...
public:
    ...
    inline int& vertexId(uint i);
    inline int& vPositionId(uint i);
    inline int& vTexcoordId(uint i);
    inline int& vNormalId(uint i);
    
protected:

    ObjFaceHandle(ObjSubMesh* pOwner, uint faceId, uint offset, uint nofVertices);
...
public:
    ...
    inline int vertexId(uint i) const;
    inline int vPositionId(uint i) const;
    inline int vTexcoordId(uint i) const;
    inline int vNormalId(uint i) const;
    
protected:

    ObjConstFaceHandle(const ObjSubMesh* pOwner, uint faceId, uint offset, uint nofVertices);
...
public:
    ...
    */
    ...
    inline ObjConstFaceHandle getFace(uint fid) const
    {
        if (mFSizes.empty())
        {
    ...
    inline ObjFaceHandle editFace(uint fid)
    {
        if (mFSizes.empty())
        {
            // means a constant number of vertices per face
    ...
    */
    ...
    ObjFaceHandle createFace(uint nofVertices, Obj::Options att, int matId=-1);
...
public:
    ...
    */
    ...
    uint getNofMaterials(void) const {return mMaterials.size();}
    ...
    uint getNofSubMeshes(void) const {return mSubMeshes.size();}
...
inline ObjFaceHandle::ObjFaceHandle(ObjSubMesh* pOwner, uint faceId, uint offset, uint nofVertices)
    : mpOwner(pOwner), mFaceId(faceId), mOffset(offset), mNofVertices(nofVertices)
{}

inline uint ObjFaceHandle::nofVertices(void) const {return mNofVertices;}
...
inline int& ObjFaceHandle::vertexId(uint i)
{
...
inline int& ObjFaceHandle::vPositionId(uint i)
{
    assert(i<nofVertices());
    uint id = mOffset + i;
    assert(id<mpOwner->mVPositionIds.size());
...
inline int& ObjFaceHandle::vTexcoordId(uint i)
{
    assert(i<nofVertices());
    uint id = mOffset + i;
...
inline int& ObjFaceHandle::vNormalId(uint i)
{
    assert(i<nofVertices());
    uint id = mOffset + i;
...
inline ObjConstFaceHandle::ObjConstFaceHandle(const ObjSubMesh* pOwner, uint faceId, uint offset, uint nofVertices)
    : mpOwner(pOwner), mFaceId(faceId), mOffset(offset), mNofVertices(nofVertices)
{}

inline uint ObjConstFaceHandle::nofVertices(void) const {return mNofVertices;}
...
inline int ObjConstFaceHandle::vertexId(uint i) const
{
    assert(i<nofVertices());
    uint id = mOffset + i;
    assert(id<mpOwner->mVPositionIds.size());
...
inline int ObjConstFaceHandle::vPositionId(uint i) const
{
    assert(i<nofVertices());
    uint id = mOffset + i;
    assert(id<mpOwner->mVPositionIds.size());
...
inline int ObjConstFaceHandle::vTexcoordId(uint i) const
{
    assert(i<nofVertices());
    uint id = mOffset + i;
...
inline int ObjConstFaceHandle::vNormalId(uint i) const
{
    assert(i<nofVertices());
    uint id = mOffset + i;
